...
class LatticeCover(BaseEstimator, TransformerMixin):
    ...
    Two kinds of cover are currently available: "triangular" and "cubical".
    The cubical cover is constructed from hypercubes whose centers 
    lie on a scaled integer root lattice :math: `mathbb{Z}^n`. 
    This will yield hypercubes of equal length in all dimensions.
    The triangular cover is generated from the dual root lattice :math: `A_n^*`,
    also known as the *triangular lattice* in dimension 2 
    and the *body-centered cubic lattice* in dimension 3. 
    The centers of all spheres from both types of covers are constructed from
    scaled generating matrices :math: `M`, and the number of points 
    in each dimension of the lattice is determined by 
    :math: `max_{1 leq i leq N+1} frac{1}{K} |M_i - m_i|`, 
    where :math:`K` is the parameter `n_intervals`.
    ...
    kind : ``'triangular'`` | ``'cubical'``, default ``'triangular'``.
        Determines the underlying root lattice for center points, 
        with ``'triangular'`` constructing :math: `A_n^*` 
        and ``'cubical'`` constructing :math: `mathbb{Z}^n`.

    special : bool, default False. 
    If `True`, `kind = 'triangular'`, and the filtered data is 2 or 3 dimensional,
    then the cover centers will be constructed with the special generating matrices
    for the $A_2^*$ and $A_3^*$ lattices.
    ...
    ball_centers_ : ndarray of shape at most ( large product see notes , dim + 1)
        ...
        Centers of every ball in the cover. The number of balls are dependent on
        `n_intervals`, `left_limits_`, and `right_limits_`.
